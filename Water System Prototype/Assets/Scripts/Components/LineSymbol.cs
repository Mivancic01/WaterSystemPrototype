...
public class LineSymbol : MonoBehaviour
{
    ...
    void OnMouseUp()
    {
        ...
        if (!GameStateManager.Instance.createPath)
        {
            //MainSimulationManager.ComponentsHelper.Instance.OpenPropertiesWindow(GetComponentInParent<ComponentObject>().elementID);
            GetComponentInParent<ComponentObject>().gameObject.GetComponent<BaseElement>().ChangeWindowVisibility(true);
